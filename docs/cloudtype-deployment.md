...
MAIGRET_BOOST_TAGS=kr:25,social:25,photo:12,video:12,blog:15,coding:15
MAIGRET_SITE_CAP_QUICK=120
...
- 무료 플랜에서는 Maigret의 동시 스캔을 크게 늘리지 않습니다. 초기값은 `MAIGRET_TOP_SITES_QUICK=100`, 태그 부스트 cap `MAIGRET_SITE_CAP_QUICK=120`, 앱 replica 1개로 시작합니다. 타임아웃이 잦으면 `MAIGRET_BOOST_TAGS` limit 또는 `MAIGRET_SITE_CAP_QUICK`을 먼저 낮춥니다.
