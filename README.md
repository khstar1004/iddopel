...
For Vercel beta, the repository `vercel.json` sets `SCAN_PROVIDER=maigret`, installs Maigret from `requirements.txt`, and routes scans through the Python function at `/api/maigret_scan` with serverless-safe Maigret limits. It also uses `/tmp` JSON stores only as a local fallback, keeps public scan responses preview-only, and limits beta free searches to 1 per request identity and browser owner token over 24 hours by default. Changing or clearing the browser owner token does not reset the request-identity quota. Change that number in `/admin`. It keeps mock payment confirmation disabled and leaves the web detailed-report paywall off. Set `MAIGRET_API_SECRET`, `REPORT_TOKEN_SECRET`, and `FIRST_FREE_FINGERPRINT_SECRET` in Vercel before public beta traffic. `/tmp` is not durable; use managed Postgres for stable detailed reports, orders, monitoring, quota settings, and deletion audits. The runtime accepts `DATABASE_URL` first, then Vercel/Neon-style `POSTGRES_URL`, `POSTGRES_PRISMA_URL`, or `POSTGRES_URL_NON_POOLING`; run `npm run db:migrate` after adding one of those env values. For Cloudtype, use the Dockerfile web/API service plus a separate Cloudtype PostgreSQL service. The native apps are still built and submitted through App Store Connect / Google Play; they point at the HTTPS backend origin via `MOBILE_APP_ORIGIN`. See [docs/cloudtype-deployment.md](docs/cloudtype-deployment.md).

Before promoting the Vercel beta to paid production, generate the redacted production env plan:
