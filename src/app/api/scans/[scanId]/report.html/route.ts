import { canAccessFullReport } from "@/lib/entitlements";
import { sanitizeMaigretHtmlReport } from "@/lib/maigret-report";
import { buildHtmlReport } from "@/lib/report-html";
import { getStoredScan } from "@/lib/scan-store";

interface RouteContext {
  params: Promise<{ scanId: string }>;
}

export async function GET(request: Request, context: RouteContext) {
  const { scanId } = await context.params;
  const scan = await getStoredScan(scanId);

  if (!scan) {
    return Response.json({ error: { code: "NOT_FOUND", message: "점검 기록을 찾을 수 없어요." } }, { status: 404 });
  }

  const url = new URL(request.url);
  const token = url.searchParams.get("token");
  const isEmbed = url.searchParams.get("embed") === "1";
  const hasAccess = await canAccessFullReport(scanId, token, request);

  if (!hasAccess) {
    return Response.json({ error: { code: "PAYMENT_REQUIRED", message: "정밀 리포트 결제가 필요해요." } }, { status: 402 });
  }

  const html = scan.maigretReport?.html
    ? sanitizeMaigretHtmlReport(scan.maigretReport.html)
    : buildHtmlReport(scan, scan.results.filter((result) => result.status === "FOUND"));
  const filename = scan.maigretReport?.htmlFilename ?? `id-doppelganger-${scan.username}.html`;

  return new Response(html, {
    headers: {
      "Content-Type": "text/html; charset=utf-8",
      "Content-Disposition": `${isEmbed ? "inline" : "attachment"}; filename="${filename}"`,
      "Content-Security-Policy":
        "default-src 'none'; img-src https: data:; style-src 'unsafe-inline'; base-uri 'none'; form-action 'none'; frame-ancestors 'self';"
    }
  });
}
